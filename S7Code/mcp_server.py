...
def _read_for_index(path: str) -> tuple[str, str]:
    ...
    if p.suffix.lower() == ".pdf":
        return _read_pdf(p), f"sandbox:{path}"
...
@mcp.tool()
def index_folder(path: str, pattern: str = "*.pdf") -> dict:
    """Index all matching files in a sandbox folder.

    Finds files under the given sandbox path using the provided pattern,
    indexes each file with `index_document`, and returns a summary containing
    the number of files found, files indexed, total chunks created, and
    per-file chunk counts.

    Example:
        index_folder("research_pprs", "*.pdf")
    """
    folder = _safe(path)
    ...
    for file in files:
        rel_path = str(file.relative_to(SANDBOX))
